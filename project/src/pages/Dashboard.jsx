...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
      {/* Welcome Section with Gradient Background */}
      <div className="relative overflow-hidden bg-yellow-300 rounded-2xl shadow-sm border border-gray-100 mb-4 sm:mb-6">
        <div className="absolute inset-0 bg-gradient-to-r from-blue-50 to-indigo-50/50" />
        <div className="relative px-4 sm:px-6 py-4 sm:py-8 md:py-12 text-center">
          <h1 className={`text-2xl sm:text-3xl md:text-4xl font-bold mb-1 mt-0 transition-all duration-600
                         animate-bounce ${colors[textColorIndex]}
                         tracking-tight leading-none flex items-center justify-center gap-2`}>
            <img src={logo} alt="RentEasy Logo" className="h-6 w-6 sm:h-8 sm:w-8 animate-spin-slow" />
            <span className="break-words">Welcome to RentEasy, {user?.displayName || user?.name || 'User'}!</span>
          </h1>
          <p className={`text-lg sm:text-xl md:text-2xl font-bold transition-all duration-900 animate-pulse
                        ${colors[(textColorIndex + 2) % colors.length]}`}>
            Effortless property management starts here today.
          </p>
          
          {/* Decorative Elements */}
          <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 w-16 sm:w-24 h-16 sm:h-24 bg-blue-500/10 rounded-full blur-2xl" />
          <div className="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/2 w-20 sm:w-32 h-20 sm:h-32 bg-indigo-500/10 rounded-full blur-2xl" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mt-4 sm:mt-8">
        {/* Left Content */}
        <div className="lg:col-span-2 space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-400">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
            <h2 className={`text-lg sm:text-xl font-semibold mb-3 sm:mb-4 transition-all duration-500
                           animate-bounce ${colors[(textColorIndex + 1) % colors.length]}`}>
              Maintenance Requests
            </h2>
            <MaintenanceRequests />
        ...
        {/* Right Sidebar */}
        <div className="lg:col-span-1 space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-500">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
            <h2 className={`text-lg sm:text-xl font-semibold mb-3 sm:mb-4 transition-all duration-500
                           animate-bounce ${colors[(textColorIndex + 4) % colors.length]}`}>
              Property Overview
